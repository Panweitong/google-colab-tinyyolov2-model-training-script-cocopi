...
parser.add_argument('--export_result_folder', default='data/custom/', type=str, 
                        help='save model')
parser.add_argument('--dataset_folder', default='data/custom/', type=str, 
                        help='use dataset')
parser.add_argument('--test', action='store_true', default=False, 
                    help='test images')
parser.add_argument('--test_result_folder', default='data/custom/', type=str, 
                        help='save test result')
parser.add_argument('--test_folder', default='data/custom/', type=str, 
                        help='test images')
    ...
    if dataset == 'voc' or dataset == "widerface" or dataset == "custom":
        for i, box in enumerate(bboxes):
            ...
            if scores[i] > thresh:
                ...
                print("Predicted ", '%s: %.2f' % (class_names[int(cls_indx)], scores[i]))
...
def test(net, device, testset, transform, thresh, class_colors=None, class_names=None, class_indexs=None, dataset='voc',test_result_folder="out/test"):
    ...
    for index in range(num_images):
        ...
        cv2.imwrite(f"{test_result_folder}/predictions.jpg", img_processed)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'voc':
        ...
        CUSTOM_ROOT = args.dataset_folder
                
        print('test on custom ...')
        class_names = CUSTOM_CLASSES
        class_indexs = None
        num_classes = len(CUSTOM_CLASSES)
        dataset = CustomDetection(root=CUSTOM_ROOT, CUSTOM_CLASSES = CUSTOM_CLASSES, image_sets=['val'], transform=None)
    ...
    if args.export:
        ...
        from convert import *
        onnx_out= args.export_result_folder + "/yolov2.onnx"
        ncnn_out_param = args.export_result_folder + "/yolov2_among.param"
        ncnn_out_bin = args.export_result_folder + "/yolov2_among.bin"
        input_shape = (3, input_size[0], input_size[1])
        import os
        if not os.path.exists(args.export_result_folder):
    ...
    if args.test:
        ...
        with torch.no_grad():
            test(net=net, 
                ...
                test_result_folder = args.test_result_folder
                )
