...
def parse_args():
    ...
    parser.add_argument('--save_folder', default='weights/', type=str,
                        help='Gamma update for SGD')
    parser.add_argument('--dataset_folder', default='data/custom/', type=str,
                        help='use dataset')
    # parser.add_argument('--classes', default=['load'], type=list,
    #                     help='use classes')
    parser.add_argument('--max_epoch', type=int,
                        default=100, help='max epoch')
...
def train():
    ...
    if args.dataset == 'voc':
        ...
        for file in os.listdir(args.dataset_folder):
            if ".names" in file:
                full_file_name = os.path.join(args.dataset_folder, file)
                f = open(full_file_name, "r")
                for each in f.readlines():
                    CUSTOM_CLASSES.append(each.strip("\n"))

        data_dir = args.dataset_folder
        num_classes = len(CUSTOM_CLASSES)
        dataset = CustomDetection(root=data_dir,
                                  CUSTOM_CLASSES=CUSTOM_CLASSES,
                                  transform=SSDAugmentation(train_size, mean=(
                                      0.5, 0.5, 0.5), std=(128/255.0, 128/255.0, 128/255.0))
                                  )
    ...
    if args.version == 'yolo_v2':
        ...
        yolo_net = myYOLOv2(device, input_size=train_size, num_classes=num_classes,
                            trainable=True, anchor_size=anchor_size, hr=hr)
        ...
        yolo_net = myYOLOv3(device, input_size=train_size, num_classes=num_classes,
                            trainable=True, anchor_size=anchor_size, hr=hr)
        ...
        yolo_net = myYOLOv3Spp(device, input_size=train_size, num_classes=num_classes,
                               trainable=True, anchor_size=anchor_size, hr=hr)
        ...
        os.environ['CUDA_VISIBLE_DEVICES'] = ",".join(
            f"{id}" for id in cards_id)

        from models.slim_yolo_v2 import SlimYOLOv2
        anchor_size = ANCHOR_SIZE if args.dataset == 'voc' else (
            ANCHOR_SIZE_COCO if args.dataset == "coco" else ANCHOR_SIZE_WIDER_FACE)

        yolo_net = SlimYOLOv2(device, input_size=train_size, num_classes=num_classes,
                              trainable=True, anchor_size=anchor_size, hr=hr)
        ...
        yolo_net = YOLOv3tiny(device, input_size=train_size, num_classes=num_classes,
                              trainable=True, anchor_size=anchor_size, hr=hr)
    ...
    if use_horovod:
        ...
        optimizer = hvd.DistributedOptimizer(
            optimizer, named_parameters=yolo_net.named_parameters())
    ...
    for epoch in range(args.start_epoch, max_epoch):
        ...
        if args.cos and epoch > 20 and epoch <= max_epoch - 20:
            # use cos lr
            tmp_lr = 0.00001 + 0.5 * \
                (base_lr-0.00001)*(1+math.cos(math.pi*(epoch-20)*1. / (max_epoch-20)))
        ...
        for iter_i, (images, targets) in enumerate(dataloader):
            ...
            if iter_i % 10 == 0:
                if args.tfboard:
                    # viz loss
                    writer.add_scalar(
                        'object loss', conf_loss.item(), iter_i + epoch * epoch_size)
                    writer.add_scalar(
                        'class loss', cls_loss.item(), iter_i + epoch * epoch_size)
                    writer.add_scalar(
                        'local loss', txtytwth_loss.item(), iter_i + epoch * epoch_size)
        ...
        if (epoch + 1):
            if not use_horovod or (use_horovod and hvd.rank() == 0):
                ...
                #            )
                if os.path.exists(os.path.join(path_to_save, args.version + '_last.pth')):
                    os.remove(os.path.join(path_to_save,
                              args.version + '_last.pth'))
                torch.save(model.state_dict(), os.path.join(path_to_save,
                                                            args.version + '_last.pth')
                           )
